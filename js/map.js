function osfStateCoordinates(name) {
    ...
    async function buildOsfMapLayers(data) {
      ...
      services.forEach(([service,group]) => {
        ...
        if (polygonData) polygonData.features.forEach(feature => {
          ...
          polygon.bindPopup(popupHtml, getOsfPopupOptions());
          polygon.addTo(layer);
        ...
        states.forEach(state => {
          ...
          severityOrder.forEach(level => {
            ...
            marker.bindPopup(markerHtml, getOsfPopupOptions());
    ...
    function osfMessageHtml(service,advisories,group,openLevel=null) {
      ...
      return levels.map(level => {
        ...
        return `<details class="osf-popup-toggle ${level}"${openLevel === level ? ' open' : ''}><summary><span>${escapeHtml(service)}</span><b>${escapeHtml(severityLabel[level])}</b></summary><div>${messageHtml}</div></details>`;
      }).join('');
    }
    ...
    function updateOsfComposite() {
      ...
      if (useCumulative) {
        osfCumulativeLayer.eachLayer(polygon => {
          ...
          polygon.bindPopup(`<div class="osf-popup"><strong>${escapeHtml(titleCase(polygon.feature.properties.District))} · ${escapeHtml(titleCase(polygon.feature.properties.STATE))}</strong><p>${sections}</p></div>` , getOsfPopupOptions());
        });
    ...
    async function buildCumulativeOsfMapLayers(data) {
      ...
      services.forEach(([service,group]) => {
        const layer = L.geoJSON(polygonData,{
          ...
          onEachFeature:(feature,polygon) => {
            ...
            polygon.bindPopup(`<div class="osf-popup"><strong>${escapeHtml(titleCase(feature.properties.District))} · ${escapeHtml(titleCase(feature.properties.STATE))}</strong>${osfMessageHtml(service,advisories,group)}</div>`, getOsfPopupOptions());
          }
        });
      ...
      osfCumulativeLayer = L.geoJSON({type:'FeatureCollection',features:[...cumulativeFeatures.values()].map(item => item.feature)}, {
        ...
        onEachFeature:(feature,polygon) => {
          ...
          polygon.bindPopup(`<div class="osf-popup"><strong>${escapeHtml(titleCase(feature.properties.District))} · ${escapeHtml(titleCase(feature.properties.STATE))}</strong><p><b>Cumulative status</b></p>${sections}</div>`, getOsfPopupOptions());
        }
      });
    ...
    if (_osfDialog) _osfDialog.addEventListener('close', () => { document.documentElement.style.overflow = ''; });

    // Popup defaults used across bindings - computed at bind-time to avoid referencing L before it exists
    function getOsfPopupOptions(){
      const padding = (window.L && typeof L.point === 'function') ? L.point(160,20) : {x:160,y:20};
      return { autoPan: true, autoPanPaddingTopLeft: padding, maxHeight: Math.round(window.innerHeight * 0.45) };
    }
