function render(data) {
      ...
      const retainedSeismic = resolveRetainedSeismicEvents(data.seismic);
      const originTime = seismicOriginDate(retainedSeismic.latest);
      ...
      latestSeismicLink.textContent = retainedSeismic.message;
      latestSeismicLink.disabled = !retainedSeismic.latest;
      latestSeismicLink.onclick = retainedSeismic.latest
        ? () => openSeismicDetails(retainedSeismic.latest, retainedSeismic.latest.bulletin ?? data.tsunami.recentBulletin)
        : null;
      ids('seismicMessageWrap').classList.toggle('is-recent',isRecentSeismic);
      ids('seismicMessageWrap').classList.toggle('is-older',Boolean(retainedSeismic.latest) && !isRecentSeismic);
      ids('seismicAdditional').replaceChildren(...retainedSeismic.recentEvents.map(event => {
      ...
      if (new URLSearchParams(location.search).get('print') === 'earthquake' && retainedSeismic.latest && !ids('seismicDialog').open) {
        openSeismicDetails(retainedSeismic.latest, retainedSeismic.latest.bulletin ?? data.tsunami.recentBulletin);
      }
