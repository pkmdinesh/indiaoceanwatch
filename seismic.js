...
const appendIst = value => {
    ...
    function seismicBulletinCount(event) {
      const raw = event?.BULNO ?? event?.bulletinCount ?? event?.bulletin?.number;
      const count = Number(raw);
      return Number.isFinite(count) && count >= 1 ? count : 1;
    }
    function seismicRetentionHours(bulletinCount) {
      const tiers = APP_CONFIG.SEISMIC_RETENTION_HOURS;
      if (bulletinCount <= 1) return tiers[1];
      if (bulletinCount === 2) return tiers[2];
      return tiers[3];
    }
    function seismicOriginDate(event) {
      const text = event?.ORIGINTIME ?? event?.originTime;
      if (!text) return null;
      return new Date(`${String(text).replace(' ','T')}+05:30`);
    }
    function isSeismicEventRetained(event, now = Date.now()) {
      const origin = seismicOriginDate(event);
      if (!origin || Number.isNaN(origin.getTime())) return false;
      const ageMs = now - origin.getTime();
      if (ageMs < 0) return false;
      return ageMs <= seismicRetentionHours(seismicBulletinCount(event)) * 60 * 60 * 1000;
    }
    function resolveRetainedSeismicEvents(seismic) {
      if (!seismic) {
        return { latest: null, recentEvents: [], message: 'No Seismic Activity for the past 90 days with magnitude >= 6.5M' };
      }
      const seen = new Set();
      const candidates = [];
      const add = event => {
        if (!event || !isSeismicEventRetained(event)) return;
        const key = event.EVID || `${event.ORIGINTIME || event.originTime}|${event.MAGNITUDE ?? event.magnitude}`;
        if (seen.has(key)) return;
        seen.add(key);
        candidates.push(event);
      };
      add(seismic.latest);
      (seismic.recentEvents || []).forEach(add);
      candidates.sort((a, b) => (seismicOriginDate(b)?.getTime() ?? 0) - (seismicOriginDate(a)?.getTime() ?? 0));
      const latest = candidates[0] ?? null;
      const recentEvents = candidates.slice(1);
      const fallbackMessage = Number(seismic.count) > 0
      ...
      return {
        latest,
        recentEvents,
        message: latest ? seismicSummary(latest) : (seismic.message || fallbackMessage)
      };
    }
